...
class Pacli extends Console_Abstract
{
    ...
    // Constants
    public const APP_URL = "https://app.asana.com";
        ...
        if (empty($body) and !is_array($body))
        {
			...
			while ($endpoint) {
				...
				if (isset($body->data)) {
					...
					if (
						isset($body->next_page)
						&& isset($body->next_page->path)
					) {
						$endpoint = substr($body->next_page->path, 1);
					}
				}
			}
			if (!empty($data)) {
    ...
    protected function setupAPI()
    {
        ...
        if (empty($api_key))
        {
            $api_key = $this->input("Enter Asana Personal Access Token (from https://app.asana.com/0/developer-console)", null, true);
